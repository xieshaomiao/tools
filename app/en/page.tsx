      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-6 pb-16 pt-12 lg:grid-cols-[1fr_0.92fr] lg:px-8 lg:pb-24 lg:pt-16">
        <div>
          <h1 className="max-w-4xl text-4xl font-black tracking-tight text-slate-950 sm:text-6xl lg:text-7xl">
            ...
            <span className="inline-block whitespace-nowrap text-blue-700">effortless</span>
          </h1>
          ...
          <form action="/en/tools" method="get" role="search" className="mt-8 flex max-w-3xl flex-col gap-3 rounded-[2rem] border border-blue-100 bg-white p-2 shadow-xl shadow-blue-950/5 sm:flex-row">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {enPopularSearches.map((item) => (
              <Link key={item.label} href={`/en/tools?q=${encodeURIComponent(item.query)}`} className="rounded-full border border-blue-100 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:text-blue-700">
                {item.label}
              </Link>
            ))}
          </div>
